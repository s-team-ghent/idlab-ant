...
class PowerProfileMessage(ProfileMessage):
    ...
    def __init__(self, msg, previous, mfg_id):
        super().__init__(msg, previous)
        self.mfg_id = mfg_id

    def __str__(self):
        return super().__str__() + ' Power: {0:.0f}W'.format(self.averagePower) + ' Cadence: {0:.0f}RPM'.format(self.instantaneousCadence)

    def json(self):
        msg = super().json()
        msg['deviceType']='power'
        msg['manufacturer_id']=self.mfg_id
        msg['sensorData']={'power': self.averagePower, 'cadence': self.instantaneousCadence, 'manufacturer_id': self.mfg_id}
        return msg
